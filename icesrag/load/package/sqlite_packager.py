...
class PackageSQLite(PackageStrategy):
    """
    Class to package documents, embeddings (the preprocessed text), and metadata for sqlite
    """
    ...
    def create_ids(self, chunks: Union[str, List[str]], 
        ...
        """
        logger.info("Creating SQLite IDs")
        ids = []
        ...
        for i, chunk in enumerate(chunks):
            ...
            if tag is None:
                ...
                if isinstance(tag, list):
                    ...
                    id = f"{tag} - {i} - {now}"
            ids.append(id)
        logger.info(f"Generated {len(ids)} SQLite IDs")
        return ids

    def process(self, chunk: str, 
                metadata: Dict,
                embedding: Optional[List[Union[str, float]]] = None, 
        ...
        Args:
            ...
            embedding (Optional[List[Union[str, float]]]): The embedding associated with the chunk.
            tag (Optional[str]): An optional tag to prepend to the IDs.
        
        Returns:
            Dict: A dictionary containing the chunk, embedding, metadata, and ID.
        """
        logger.info("Processing single chunk for SQLite")
    ...
    def batch_process(self, 
        ...
        Args:
            ...
            embeddings (Optional[List[List[Union[str, float]]]]): A list of embeddings, each corresponding to a chunk.
                a. for SQL, should only ever be list[str], not 
        ...
        """
        logger.info(f"Processing batch of {len(chunks)} chunks for SQLite")
        documents = []
        embeds = []
        mdata = []
        if ids is None:
